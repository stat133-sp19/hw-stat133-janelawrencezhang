# library(testthat)
# test_file("tests/testthat/tests_binomial.R")
# test_file("tests/testthat/tests_checkers.R")
# test_file("tests/testthat/tests_summary.R")
...
#A private auxiliary function "mean" that calculate the mean based on the inputs, tirals and prob
.aux_mean <- function(trials,prob){
  .check_trials(trials)
  .check_prob(prob)
  return(trials*prob)
}
#A private auxiliary function "variance" that calculate the variance based on the inputs, tirals and prob
...
#A private auxiliary function "mode" that calculate the mode based on the inputs, tirals and prob
...
#A private auxiliary function "skewness" that calculate the skewness based on the inputs, tirals and prob
...
#A private auxiliary function "kurtosis" that calculate the kurtosis based on the inputs, tirals and prob
...
bin_choose <- function(n,k){
  logi <- n>=k
  if (all(logi, k>=0 , n>0 , k%%1==0 , n%%1==0)){
    numerator <- factorial(n)
    de1 <- factorial(k)
    y <- n-k
    de2 <- factorial(y)
    return(numerator/(de1*de2))
  }else if (k>n & k>=0 & n>0 & k%%1==0 & n%%1==0 ){
...
plot.bindis <- function(x){
  library(ggplot2)
  graph <- ggplot(data=x,aes(x=factor(success),y=probability)) +
    geom_bar(stat='identity',fill="grey") +theme_light()+
    scale_x_discrete(breaks=0:max(x$success), labels=0:max(x$success) ) + xlab("successes")
  return(graph)
}
...
plot.bincum <- function(x){
  library(ggplot2)
  graph <- ggplot(data=x,aes(x=success,y=cumulative))+
